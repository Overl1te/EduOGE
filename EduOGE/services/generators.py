...
def pick_multiple(rng: random.Random, *, base: int, lo: int, hi: int) -> int:
    """
    Выбирает число в [lo, hi], кратное base.
    """
    lo_k = (lo + base - 1) // base
    hi_k = hi // base
    if lo_k > hi_k:
        # fallback: просто hi округлим вниз
...
def ensure_divisible_in_list(rng: random.Random, data: List[int], d: int) -> None:
    """
    Гарантирует, что в списке есть хотя бы одно число, кратное d.
    """
    idx = rng.randrange(0, len(data))
    max_val = max(data)
    # аккуратно подберём кратное d в диапазоне 1..max(data) (если вдруг max слишком маленький, расширяем)
    upper = max(max_val, d)
...
def pick_count_limit_mult_5(rng: random.Random) -> int:
    # “20 (на кратные 5)” -> берём разумный набор
...
class SumDivisibleNList:
    """
    Пример-образец: SumDivisibleBy6, но теперь делитель и ограничения рандомные.
    """
...
class MinDivisibleGuaranteedNList:
    """
    Образец: MinEven. Теперь ищем минимальное кратное d и гарантируем наличие.
    """
...
class SumAndCountDivisibleNList:
    """
    Образец: SumAndCountEven. Теперь: сумма и количество кратных d.
    """
...
class MaxAndYesNoDivisibleNList:
    """
    Образец: MaxAndYesNo (кратно 5). Теперь кратность по d.
    """
...
class AvgDivisibleOrNoNList:
    """
    Образец: AvgAndCheck (кратно 3). Теперь кратность по d.
    """
...
class UntilZeroAvg:
    """
    Образец: UntilZeroAvg. Параметризуем лимиты.
    """
    key = "until_zero_avg"
...
class RangeCountDivisible:
    """
    Образец: CountEvenInRange. Теперь делимость по d и лимиты рандомные.
    """
    ...
    def sample_params(self, rng):
        d = pick_divisor_2_10(rng)
        v_max = pick_value_limit_mult_500(rng)
        delta_max = pick_count_limit_mult_5(rng)  # "длина" отрезка тоже кратная 5
...
class SumBaseLastDigitNList:
    """
    Образец: "в 7-ричной оканчивается на 1".
    Теперь: основание base (2..10) и последняя цифра last (0..base-1).
    Условие эквивалентно x % base == last.
    """
    ...
    def make_statement(self, p):
        return normalize(f"""
Напишите программу подсчёта суммы элементов последовательности натуральных чисел, запись которых в {p["base"]}-ричной системе счисления
оканчивается на цифру {p["last"]}. В ответе запишите только сумму.

На вход программе сначала подается количество элементов последовательности N (1 ≤ N ≤ {p["n_max"]}),
затем каждый элемент последовательности в отдельной строке. Введённые числа по модулю не превышают {p["v_max"]}.

Программа должна вывести одно число: сумму подходящих элементов (в десятичной системе счисления).
""")
...
class SumThreeDigitDivisibleUntilZero:
    ...
    Меняем:
      - делитель d (2..10)
      - count_max (кратные 5)
      - value_max (кратные 500)
    """
    key = "sum_3digit_divisible_until_zero"

    def sample_params(self, rng):
        d = pick_divisor_2_10(rng)
        count_max = pick_count_limit_mult_5(rng)
        v_max = pick_value_limit_mult_500(rng)
        # чтобы “трёхзначные” имели смысл, гарантируем минимум 999 доступен не всегда, но это ок:
        # если v_max < 100, сумма всегда 0. Но v_max у нас минимум 500, так что всё нормально.
...
Программа должна вывести одно число: сумму трехзначных чисел, кратных {p["d"]}.
""")
...
def generate_task(seed: int) -> GeneratedTask:
    ...
    # анти-угадайка: несколько тестов с одинаковыми параметрами задачи
    tests: List[TestCase] = []
    for _ in range(5):
    ...
    return GeneratedTask(
        ...
        tests=tests,
        solution_py=normalize(family.make_solution_py(params)),
        solution_js=normalize(family.make_solution_js(params)),
    )